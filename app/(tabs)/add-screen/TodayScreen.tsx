...
import { DEFAULT_REPS, DEFAULT_SET_COUNT, MAX_SET_COUNT, MIN_SET_COUNT, REP_OPTIONS, WEIGHT_OPTIONS, type ExerciseSetDraft } from './todayWorkoutConstants';
          ...
          <View style={styles.createModal}>
            ...
            <ScrollView
              ...
              <TouchableOpacity style={styles.checkboxRow} onPress={() => setAddToSavedExercises(prev => !prev)}>
                <Text style={styles.checkboxIcon}>{addToSavedExercises ? 'â˜‘' : 'â˜'}</Text>
...
export default function AddScreen() {
  ...
  return (
    <View style={styles.todayContainer}>
                  ...
                  <View key={exercise.id} style={styles.todayExerciseRow}>
                    <TouchableOpacity
                      ...
                      <View style={[styles.sessionCheckbox, checked && styles.sessionCheckboxChecked]}>
                        {checked ? <Text style={styles.sessionCheckboxCheck}>âœ“</Text> : null}
                      </View>
                    </TouchableOpacity>
        ...
        {assignedWorkout ? (
          <TouchableOpacity
            ...
            <Text style={[styles.completeButtonText, isCompletedToday && styles.completeButtonTextCompleted]}>
              {isCompletedToday ? 'Completed âœ“' : 'Workout Completed'}
            </Text>
          </TouchableOpacity>
        ) : null}
      </View>
      ...
      <ExerciseLogModal visible={exerciseLogVisible} transparent animationType="fade" onRequestClose={closeExerciseLog}>
        <View style={styles.modalOverlay}>
          <View style={styles.exerciseLogModalContent}>
            ...
            {/* â”€â”€ REMOVE SET PICKER VIEW â”€â”€ */}
            {logView === 'pick-remove' && (
              <>
                  ...
                  {draftSets.slice(0, draftSetCount).map((_, index) => {
                    ...
                    return (
                      <TouchableOpacity
                        ...
                        <Text style={styles.removeSetRowText}>{selected ? 'â˜‘' : 'â˜'}</Text>
                      </TouchableOpacity>
                    );
                  })}
                </ScrollView>
